...
class UmaApp:
    ...
    def create_ui(self):
        ...
        try:
            self.original_image = Image.open("/home/workdir/attachments/screenshot_2026-06-30_00-09-00.jpg")
            self.update_image()
        except Exception:
            pass
    ...
    def update_period_summary(self):
        ...
        for row_idx, (pname, (start, end)) in enumerate(periods.items(), start=1):
            ...
            for col_idx, t in enumerate(training_types, start=1):
                ...
                tk.Label(cell_frame, text="" if t in ("お出かけ","お休み","レース") else f"({pval})", font=("Helvetica", 9, "bold"), foreground="#cc0000" if pval > 0 else "#666666", background=color, anchor="center", width=4).pack(side="left")
        # 合計行
        ...
        for col_idx, t in enumerate(training_types, start=1):
            ...
            tk.Label(cell_frame, text="" if t in ("お出かけ","お休み","レース") else f"({pval})", font=("Helvetica", 9, "bold"), foreground="#cc0000" if pval > 0 else "#666666", background=color, anchor="center", width=4).pack(side="left")        # ラーメン集計
